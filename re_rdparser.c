...
past Number()
{
    ...
    {
        past tmp = newNumber(yylval, left, right);
        advance();
        return tmp;
    }
    return NULL;
...
past BType()
{
    ...
    if (token == tok_INT)
    {
        past tmp = newBType(yylval, left, right);
        advance();
        return tmp;
    }
    return NULL;
...
void showSub(past node, int nest)
{
    if (node == NULL)
    {
        return;
    ...
    // if (node->right == NULL)
    // {
    //     showSub(node->left, nest);
    // }
    showSub(node->left, nest + 1);
    showSub(node->right, nest + 1);
}

void showAst(past mainNode)
{
    past tmp = mainNode;
    printf("%s:\n", tmp->nodeType);
